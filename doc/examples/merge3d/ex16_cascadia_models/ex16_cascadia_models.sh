...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/gq-merge3d-ex16-cascadia.XXXXXX")
...
plot_horizontal() {
	local ps_file="${work_dir}/ex16_cascadia_models_horizontal.ps"
	...
	gmt psconvert "${ps_file}" -A+m0.1i -Tf \
		-F"${script_dir}/ex16_cascadia_models_horizontal"
	gmt psconvert "${ps_file}" -A+m0.1i -Tg -E300 \
		-F"${script_dir}/ex16_cascadia_models_horizontal"
}

plot_vertical() {
	local ps_file="${work_dir}/ex16_cascadia_models_vertical.ps"
	local grids=(tiled merged aggregate)
	local titles=("Tiling" "Cosine merging" "Aggregate merging")
	local letters=(a b c)
	...
	gmt psscale -R -J -C"${velocity_cpt}" \
		...
		-O >> "${ps_file}"
	gmt psconvert "${ps_file}" -A+m0.1i -Tf \
		-F"${script_dir}/ex16_cascadia_models_vertical"
	gmt psconvert "${ps_file}" -A+m0.1i -Tg -E300 \
		-F"${script_dir}/ex16_cascadia_models_vertical"
}
