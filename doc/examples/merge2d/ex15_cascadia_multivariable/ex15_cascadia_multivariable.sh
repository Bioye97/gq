...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/gq-merge2d-ex15-cascadia.XXXXXX")
...
cat > "${vp_vs_mergefile}" <<- EOF
	${svi_vp_vs}?Band1,Band2 ${wus324_vp_vs}?Band1,Band2 - cosine/cosine 0.2
	${casc16_vp_vs}?Band1,Band2 ${wus324_vp_vs}?Band1,Band2 - cosine/cosine 0.2
...
cat > "${vp_vs_density_mergefile}" <<- EOF
	${svi_vp_vs_density}?Band1,Band2,Band3 ${wus324_vp_vs_density}?Band1,Band2,Band3 - cosine/cosine 0.2
	${wus324_vp_vs_density}?Band1,Band2,Band3 - - - -
	EOF
...
projection=M2.55i
ps_file="${work_dir}/ex15_cascadia_vp_vs.ps"
...
gmt psconvert "${ps_file}" -A -Tf -F"${script_dir}/ex15_cascadia_vp_vs"
gmt psconvert "${ps_file}" -A -Tg -E300 \
	-F"${script_dir}/ex15_cascadia_vp_vs"

# Figure 2: Vp, Vs, and density from SVI EQTOMO and WUS324.
projection=M2.2i
ps_file="${work_dir}/ex15_cascadia_vp_vs_density.ps"
...
gmt psconvert "${ps_file}" -A -Tf \
	-F"${script_dir}/ex15_cascadia_vp_vs_density"
gmt psconvert "${ps_file}" -A -Tg -E300 \
	-F"${script_dir}/ex15_cascadia_vp_vs_density"
