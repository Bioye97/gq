...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/gq-merge2d-ex17-cascadia.XXXXXX")
...
make_comparison() {
	...
	cat > "${mergefile}" <<- EOF
		...
		${primary} ${wus324} - cosine/cosine 0.49
		${wus324} - - - -
		EOF
...
make_comparison "${ant_rf}" ex17_cascadia_ant_rf
make_comparison "${casc16}" ex17_cascadia_v16
