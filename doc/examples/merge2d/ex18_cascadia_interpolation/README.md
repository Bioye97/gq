# ex18_cascadia_interpolation

This high-taper example repeats `ex14_cascadia_interpolation` with taper
ratios of 0.49.

This example repeats the SVI EQTOMO, Cascadia ANT+RF, and WUS324 comparison
from `ex17_cascadia_models`, but uses `-Hl` to fill all strictly internal gaps
...
```bash
./ex18_cascadia_interpolation.sh
```

The script creates `ex18_cascadia_interpolation.png` and
`ex18_cascadia_interpolation.pdf`.
