...
This example removes the existing topography from the Cascadia ANT+RF
shear-wave velocity model with `-Or+t` and then adds dry-land topography
to that flattened model with `-Oa+t` using each of the three `topobath` construction
methods. Wet columns remain outside the selected operation scope.
...
- `ex01_ant_rf_diagnostics.png` and `ex01_ant_rf_diagnostics.pdf`
- `ex01_ant_rf_methods.png` and `ex01_ant_rf_methods.pdf`
