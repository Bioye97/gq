#!/usr/bin/env bash
#
# Flatten Cascadia ANT+RF and compare three topography construction methods.
...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/gq-topobath-ex01-ant-rf.XXXXXX")
...
# Infer and write the existing surface, then shift it to zero depth. Retain
# enough depth for the subsequent pull/push operation.
...
diagnostics_ps="${work_dir}/ex01_ant_rf_diagnostics.ps"
...
gmt psconvert "${methods_ps}" -A+m0.1i -Tf \
	-F"${script_dir}/ex01_ant_rf_methods"
