...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/gq-merge2d-ex20-cascadia.XXXXXX")
...
increment=0.05
projection=M2.2i
ps_file="${work_dir}/ex20_cascadia_polygon_supports.ps"
...
cat > "${mergefile}" <<- EOF
	${svi} ${wus324} ${star} cosine/cosine 0.49
	${ant_rf} ${wus324} ${triangle} cosine/cosine 0.49
	${wus324} - - - -
	EOF
...
gmt psconvert "${ps_file}" -A -Tf \
	-F"${script_dir}/ex20_cascadia_polygon_supports"
