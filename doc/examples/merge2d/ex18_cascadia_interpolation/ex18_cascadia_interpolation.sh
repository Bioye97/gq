...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/gq-merge2d-ex18-cascadia.XXXXXX")
...
increment=0.05
projection=M2.2i
ps_file="${work_dir}/ex18_cascadia_interpolation.ps"
velocity_cpt="${work_dir}/velocity.cpt"
colorbar_font=9.8p

cat > "${mergefile}" <<- EOF
	${svi} ${wus324} - cosine/cosine 0.49
	${ant_rf} ${wus324} - cosine/cosine 0.49
	${wus324} - - - -
	EOF
...
gmt psconvert "${ps_file}" -A -Tf -F"${script_dir}/ex18_cascadia_interpolation"
gmt psconvert "${ps_file}" -A -Tg -E300 \
	-F"${script_dir}/ex18_cascadia_interpolation"
