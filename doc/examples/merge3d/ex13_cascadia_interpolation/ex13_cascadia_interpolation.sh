...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/gq-merge3d-ex13-cascadia.XXXXXX")
...
plot_horizontal() {
	local ps_file="${work_dir}/ex13_horizontal.ps"
	...
	gmt psconvert "${ps_file}" -A+m0.1i -Tf \
		-F"${script_dir}/ex13_cascadia_interpolation_horizontal"
	gmt psconvert "${ps_file}" -A+m0.1i -Tg -E300 \
		-F"${script_dir}/ex13_cascadia_interpolation_horizontal"
}

plot_vertical() {
	local ps_file="${work_dir}/ex13_vertical.ps"
	local names=(tiled merged aggregate)
	local titles=("Tiling" "Merging" "Aggregate")
	local letters=(a b c)
	for index in 0 1 2; do
	...
	gmt psscale -R -J -C"${velocity_cpt}" \
		...
		-O >> "${ps_file}"
	gmt psconvert "${ps_file}" -A+m0.1i -Tf \
		-F"${script_dir}/ex13_cascadia_interpolation_vertical"
	gmt psconvert "${ps_file}" -A+m0.1i -Tg -E300 \
		-F"${script_dir}/ex13_cascadia_interpolation_vertical"
}
