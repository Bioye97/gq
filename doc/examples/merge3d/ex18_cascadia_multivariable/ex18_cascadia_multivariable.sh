...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/gq-merge3d-ex18-cascadia.XXXXXX")
...
plot_horizontal() {
	local ps_file="${work_dir}/ex18_horizontal.ps"
	...
	gmt psconvert "${ps_file}" -A+m0.1i -Tf \
		-F"${script_dir}/ex18_cascadia_multivariable_horizontal"
	gmt psconvert "${ps_file}" -A+m0.1i -Tg -E300 \
		-F"${script_dir}/ex18_cascadia_multivariable_horizontal"
}

plot_vertical() {
	local ps_file="${work_dir}/ex18_vertical.ps"
	for index in 0 1 2 3 4 5; do
		field=${fields[index]}
		state=${states[index]}
		cpt=$(field_cpt "${field}")
	...
	gmt psxy -R -J -T -O >> "${ps_file}"
	gmt psconvert "${ps_file}" -A+m0.1i -Tf \
		-F"${script_dir}/ex18_cascadia_multivariable_vertical"
	gmt psconvert "${ps_file}" -A+m0.1i -Tg -E300 \
		-F"${script_dir}/ex18_cascadia_multivariable_vertical"
}
