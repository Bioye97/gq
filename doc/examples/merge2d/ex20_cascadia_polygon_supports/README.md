# ex20_cascadia_polygon_supports

This high-taper example repeats `ex16_cascadia_polygon_supports` with taper
ratios of 0.49.

This example repeats the interpolated Cascadia comparison from
`ex18_cascadia_interpolation`, but neither primary uses its entire grid domain.
...
```text
SVI_EQTOMO WUS324 svi_star.txt cosine/cosine 0.49
Cascadia_ANT+RF WUS324 ant_rf_triangle.txt cosine/cosine 0.49
WUS324 - - - -
```
...
```bash
./ex20_cascadia_polygon_supports.sh
```

The script creates `ex20_cascadia_polygon_supports.png` and
`ex20_cascadia_polygon_supports.pdf`.
